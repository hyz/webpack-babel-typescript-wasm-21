...
use crate::game::utils::{player_number_match, PlayerNumber};
...
impl Manager {
    pub fn new(name1: String, name2: String) -> Self {
        ...
        Manager {
            ...
            player2,
            board,
            turn: Manager::random_turn(),
        }
    }

    pub fn move_gobblet_from_hand_to_board(
        &mut self,
        coord: &Coord,
        quadrant: u8,
    ) -> Option<Gobblet> {
    ...
    fn random_turn() -> PlayerNumber {
        return if unsafe { Math::random() } > 0.5 {
            PlayerNumber::One
        } else {
            PlayerNumber::Two
        };
